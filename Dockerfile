...
RUN a2enmod rewrite headers \
    && a2ensite crud-laravel \
    && a2dissite 000-default \
    && chmod +x /usr/local/bin/docker-crud-laravel-entrypoint

COPY . /var/www/crud-laravel
RUN composer install --optimize-autoloader --no-dev

CMD ["docker-crud-laravel-entrypoint"]
